...
describe('real A2A HTTP transport', () => {
  it('discovers the card and returns a completed structured Task artifact', async () => {
    ...
    expect(outcome.result.trends.length).toBeGreaterThan(0)
    const evidenceIds = new Set(outcome.result.evidence.map((item) => item.id))
    expect(outcome.result.metrics.every((metric) => metric.evidenceIds.every((id) => evidenceIds.has(id)))).toBe(true)
  ...
  it('serves all three cards and executes Market and Technology over A2A', async () => {
    ...
    expect(market.result.findings.length).toBeGreaterThan(0)
    expect(market.result.evidence.length).toBeGreaterThan(0)
    expect(technology.result.dimension).toBe('technology')
    expect(technology.result.findings.length).toBeGreaterThan(0)
    expect(technology.result.evidence.length).toBeGreaterThan(0)
  })
})
