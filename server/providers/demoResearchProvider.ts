...
function findCompany(raw: string): ResearchCompany {
  const key = normalizeLookup(raw)
  if (!key) throw new ResearchProviderError('Missing company name / ticker', 'BAD_ARGS')

  const direct = COMPANIES[ALIASES[key] ?? key]
  if (direct) return direct

  const fuzzy = Object.values(COMPANIES).find(
    (company) =>
      company.profile.name.toLowerCase().includes(key) ||
      company.profile.ticker.toLowerCase().includes(key),
  )
  if (fuzzy) return fuzzy

  throw new ResearchProviderError(`Unknown company: "${raw}"`, 'NOT_FOUND')
}

/**
 * Demo provider backed by deterministic in-repository fixtures.
 *
 * It is intentionally isolated behind ResearchDataProvider so replacing it with
 * a live API/database provider does not change MCP, Agent, A2A or A2UI layers.
 */
export class DemoResearchProvider implements ResearchDataProvider {
  readonly metadata = {
    ...
    kind: 'demo' as const,
    sourceLabel: 'MCP Research Tool (Demo Data)',
  }
