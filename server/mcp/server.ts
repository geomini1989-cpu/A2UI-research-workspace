...
/**
 * Build the Research MCP server exposing provider-backed research tools.
 *
...
export function createResearchServer(): McpServer {
  const server = new McpServer({ name: 'research-provider-adapter', version: '1.1.0' })

  register(server, 'search_company', 'Search companies', 'Search the configured research provider by company name or ticker.', searchCompanyArgs)
...
function register(
  ...
  server.registerTool(name, { title, description, inputSchema : inputSchema }, async (args) => {
    try {
      const result = await executeResearchTool(name, args)
      return {
